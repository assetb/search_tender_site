...
//          alert('i have come in foundationSelect');
        selectPrompt = '';
        selected = '';
        translateClasses = '';
        select = $(this);
  ...
  $.fn.foundationSelectOrigin = function() {
  ...
  $.fn.foundationSelectPromptCorrected = function() {
    ...
    if (!$('.custom-dropdown-area').length) {
      ...
      return this.each(function () {
        ...
        select.find('option').each( function () {
          if ($(this).attr('selected')) {
            selected = 'selected';
            selectpromptCollection = selectPromptCollection + ', ' + $(this).html(); 
            selectPrompt = "<div class='" + $(this).attr('class') + "'>" + selectpromptCollection + "</div>";
          }
          if( $(this).attr('class') ) {
            translateClasses = $(this).attr('class') + ' ';
          }
    ...
    if (multiple) {
      ...
      if (selectedOptions.length) {
        if (selectedOptions.length > 3) {
          dropdown.find('.custom-dropdown-button').html(selectedOptions.length + ' из ' + totalOptions + ' выбрано');
        }else{
